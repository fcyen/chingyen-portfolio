...
src/
  routes/
    Home/                  # the character-select stage (owns frame, persona state, full-bleed bg)
    Work/[slug].tsx        # case-study template
...
To be filled in once stage 1 (scaffold) is done. Expected:

```bash
npm install
npm run dev          # Vite dev server
npm run build        # production build (runs scripts/fetch-substack.mjs first)
