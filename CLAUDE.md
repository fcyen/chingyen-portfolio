...
- **Mobile Landing fonts match desktop**: `.mHeroTitle` and `.mCardName` on the Landing mobile layout both use `var(--title-display)` (Jersey 10), the same pixel/arcade font as desktop. Do not substitute `var(--serif)` or any other font for the mobile hero title or card names.
